...
def load_csv_signage(path, post_processed=False):
    if post_processed:
        return pd.read_csv(path, parse_dates=[-3, -2],
                           date_parser=lambda x: pd.datetime.strptime(x, '%Y:%m:%d %H:%M:%S.%f'))
    else:
        return pd.read_csv(path, parse_dates=[-7, -6],
                           date_parser=lambda x: pd.datetime.strptime(x, '%Y:%m:%d %H:%M:%S.%f'))
...
def post_processing_signage_csv(input_csv, output_csv, signage_enter_area, time_diff_threshold=5.0,
                                accompany_iou_threshold=5.0):
    ...
    for index1, row1 in group_df.iterrows():
        ...
        for index2, row2 in group_df.iterrows():
            ...
            time_diff = (row2['Start_time'] - signage_df.at[
                signage_df.index.values[index1], 'End_time'] + time_offset).total_seconds()
            dist = distance.euclidean((float(signage_df.at[signage_df.index.values[index1], 'End_bbox_x']),
                                       float(signage_df.at[signage_df.index.values[index1], 'End_bbox_y'])),
                                      (float(row2['Start_bbox_x']), float(row2['Start_bbox_y'])))
            center_point = Point(float(row2['Start_bbox_x']), float(row2['Start_bbox_y']))
            if time_diff <= time_diff_threshold and time_diff >= 0 and dist < min_dist and not signage_enter_area.contains(
                    center_point) and (signage_df.at[signage_df.index.values[index2], 'assigned'] == 0):
                min_dist = dist
                min_index = index2
                print('ID {} matched with {}'.format(signage_df.at[signage_df.index.values[index1], 'shopper ID'],
                                                     signage_df.at[signage_df.index.values[min_index], 'shopper ID']))
                ...
                signage_df.at[signage_df.index.values[index1], 'End_time'] = signage_df.at[
                    signage_df.index.values[min_index], 'End_time']
                min_dist = sys.maxsize
                min_index = None
        ...
        if len(enter_sig1_df) > 0 and len(leave_sig1_df) > 0:
            fisheye_sig1_tracks.append((identity, datetime.strptime(
                fisheye_df.at[enter_sig1_df.index.values[0], 'timestamp (UTC - JST)'], "%Y:%m:%d %H:%M:%S.%f"),
                                        datetime.strptime(
                                            fisheye_df.at[leave_sig1_df.index.values[-1], 'timestamp (UTC - JST)'],
                                            "%Y:%m:%d %H:%M:%S.%f")))
        if len(enter_sig2_df) > 0 and len(leave_sig2_df) > 0:
            fisheye_sig2_tracks.append((identity, datetime.strptime(
                fisheye_df.at[enter_sig2_df.index.values[0], 'timestamp (UTC - JST)'], "%Y:%m:%d %H:%M:%S.%f"),
                                        datetime.strptime(
                                            fisheye_df.at[leave_sig2_df.index.values[-1], 'timestamp (UTC - JST)'],
                                            "%Y:%m:%d %H:%M:%S.%f")))
    ...
    if len(fisheye_sig1_tracks) > 0 and len(signage1_tracks) > 0:
        ...
        for r, c in zip(matched_row, matched_col):
            if IoU_mat[r, c] >= iou_threshold:
                ...
                print('>>> [SIGNAGE 1] Fisheye id {} matches with signage id {} with time IoU of {}'.format(
                    fisheye_sig1_tracks[r][0], signage1_tracks[c][0], IoU_mat[r, c]))
        ...
        for idx, trk in enumerate(signage1_tracks):
            ...
            if len(attention_list) > 0:
                process_ID = 1557
                attention_df = signage1_df.loc[
                    (signage1_df['shopper ID'] == trk[0]) & signage1_df['info'].str.contains('attention')]
    ...
    if len(fisheye_sig2_tracks) > 0 and len(signage2_tracks) > 0:
        ...
        for r, c in zip(matched_row, matched_col):
            if IoU_mat[r, c] >= iou_threshold:
                ...
                print('>>> [SIGNAGE 2] Fisheye id {} matches with signage id {} with time IoU of {}'.format(
                    fisheye_sig2_tracks[r][0], signage2_tracks[c][0], IoU_mat[r, c]))
                for _, row in cur_sig_df.iterrows():
                    process_ID = None
                    info = None
                    if 'GROUP' in row['info']:
                    ...
                    fisheye_df = fisheye_df.append(
                        {'camera ID': 2, 'shopper ID': fisheye_sig2_tracks[r][0], 'process ID': process_ID,
                         'info': info,
                         'timestamp (unix timestamp)': time.mktime(row['Start_time'].timetuple()),
                         'timestamp (UTC - JST)': row['Start_time']}, ignore_index=True)
        ...
        for idx, trk in enumerate(signage2_tracks):
            ...
            if len(attention_list) > 0:
                process_ID = 1557
                attention_df = signage2_df.loc[
                    (signage2_df['shopper ID'] == trk[0]) & signage2_df['info'].str.contains('attention')]

                for _, row in attention_df.iterrows():
                    info = 'HAS ATTENTION TO SIGNAGE_2 IN {}s'.format(row['Duration'])

                    fisheye_df = fisheye_df.append(
                        {'camera ID': 2, 'shopper ID': str(attention_list), 'process ID': process_ID,
                         'info': info,
                         'timestamp (unix timestamp)': time.mktime(row['Start_time'].timetuple()),
                         'timestamp (UTC - JST)': row['Start_time']}, ignore_index=True)
